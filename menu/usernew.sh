...
read -p "limit IP: " max
folder="/etc/cybervpn/limit/ssh/ip/"
if [ ! -d "$folder" ]; then
    mkdir -p "$folder"
else
    echo ""
...
echo -e "PATCH /ssh-cybervpn HTTP/1.1[crlf]Host: [host][crlf]Host: ISIBUG[crlf]Upgrade: websocket[crlf]Connection: Upgrade[crlf]User-Agent: [ua][crlf][crlf]"
